<script>
	...
	onMount(async () => {
		...
		// 2. Load Profile (Tetap jalan biar data diri update)
		await loadUserData();

		// 3. LOAD DUMMY STATS (Langsung dipanggil!)
		loadDummyStats();
	});
		...
		try {
			...
			if (res.ok) {
				...
				user = {
					...
					photo_url: d.photo_url || user.photo_url,
				...
				if (d.preferences) {
					personalData = {
						hobbies: d.preferences.hobbies || personalData.hobbies || '',
						favorite_foods: d.preferences.favorite_foods || personalData.favorite_foods || '',
						allergies: d.preferences.allergies || personalData.allergies || '',
						health_goals: d.preferences.health_goals || personalData.health_goals || ''
					};
				}
				const storagePayload = { ...user, preferences: personalData };
				localStorage.setItem('user_data', JSON.stringify(storagePayload));
	...
	// --- LOGIC STATISTIK (DATA DUMMY YANG KAMU KASIH) ---
				...
				history: [
					{
						...
						// score gak ada, nanti dihitung manual
					},
					{
						date: '2026-01-21',
						mood: null,
					...
					{
						date: '2026-01-23', // Tanggal disesuaikan biar urut
						mood: null,
						mood_score: 3,
						physical_symptoms: null
					},
					{
						date: '2026-01-24', // Tanggal disesuaikan biar urut
						mood: null,
						mood_score: 2,
						physical_symptoms: null
					},
					{
						date: '2026-01-25', // Tanggal disesuaikan biar urut
						mood: 'senang',
						mood_score: 5,
						physical_symptoms: 'bahagia'
					},
					{
						date: '2026-01-26', // Hari ini
						mood: 'senang',
						mood_score: 4,
						score: 85, // Ada score eksplisit
						physical_symptoms: 'bahagia'
					}
				],
				mood_summary: {
	...
	function processStats(data) {
		...
		if (data.history) {
			...
			weeklyStats = last7Days.map((item) => {
				...
				// Logic: Jika ada 'score', pakai itu. Jika tidak, hitung dari mood_score (x20)
		...
		if (data.mood_summary) {
			moodSummaryList = moodSummaryList.map((item) => ({
				...item,
				// Ambil count dari dummy data.
				// Note: key di dummy kamu "senang", "bahagia".
				count: data.mood_summary[item.key] || 0
			}));
		}
	}

	// --- FUNGSI EDIT PROFILE & PERSONAL (Sama seperti sebelumnya) ---
	...
	function validateProfileForm() {
		if (!profileForm.name || !profileForm.diagnosis) return false;
		return true;
	}

	async function saveProfile() {
		if (!validateProfileForm()) return alert('Nama & Diagnosis wajib diisi!');
		isLoading = true;
		const token = localStorage.getItem('auth_token');
		try {
	...
	async function savePersonal() {
		...
		try {
			...
			currentData.preferences = personalData;
	...
	<main class="relative z-20 mx-auto -mt-20 max-w-4xl space-y-6 px-6 md:px-10">
			...
			<div class="flex flex-col items-center text-center">
				...
				{#if isEditingProfile}
					<div class="w-full space-y-4 px-2 text-left" in:slide>
						...
						<div class="mb-6 text-center">
							<label class="text-[10px] font-bold text-slate-400 uppercase">Diagnosis</label><Input
		...
		<Card className="relative border-none shadow-md">
			<div class="mb-4 flex items-center justify-between">
				<div>
					<h3 class="text-lg font-bold text-slate-800">Agar Kami Lebih Mengenalmu ✨</h3>
			...
			{#if isEditingPersonal}
				<div class="space-y-4" in:slide>
					...
					<div>
						<label class="text-[10px] font-bold text-slate-400 uppercase">Goals</label><textarea
			...
			{:else}
				<div class="grid grid-cols-1 gap-3 text-sm">
					...
					<div class="flex items-start gap-3 rounded-xl bg-slate-50 p-3">
						<span class="text-lg">🎯</span>
						<div>
							<span class="block text-[10px] font-bold text-slate-400 uppercase">Goals</span>
			...
			<div class="flex items-end justify-between px-2 sm:px-6">
				{#each moodSummaryList as mood}
					<div class="flex flex-col items-center gap-2 transition-transform hover:scale-110">
						...
						<div
							class="flex h-6 min-w-[24px] items-center justify-center rounded-full px-2 text-xs font-bold transition-colors
                            {mood.count > 0
